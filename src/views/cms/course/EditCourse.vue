...
watch(dynamicTagsValue, () => {
  ...
  if (dynamicTagsValue.value.length > courseData.value.courseList.length) {
    courseData.value.courseList.push({
      ctag: {
        courseTagName:
          dynamicTagsValue.value[dynamicTagsValue.value.length - 1],
      },
    })
    console.log("add", courseData.value.courseList)
  } else if (
    dynamicTagsValue.value.length < courseData.value.courseList.length
  ) {
...
onBeforeMount(async () => {
  ...
  if (result.data != null) {
    ...
    courseData.value.courseDate = new Date(
      courseData.value.courseDate
    ).getTime()
...
<template>
  <div v-cloak v-if="courseData != null" class="justifyContainer">
    <div class="picDisplayContainer">
      <div class="picDisplay">
        <img
          :src="
            courseData.coursePictureList[selectedImg]
              ? courseData.coursePictureList[selectedImg].courseImgURL
              : '/images/navbar/image-gallery.png'
          "
        />
      </div>
      <div class="allPicContainer">
        <div
          v-for="(img, index) in courseData.coursePictureList"
          :key="img.id"
          :class="
            index == selectedImg ? `imgDisplay selectedPic` : `imgDisplay`
          "
        ...
        >
          <div v-if="updateDisabled" class="deleteContain">
            <span class="dlelBtnContainer">
              <font-awesome-icon
                @click.stop="deletePic(img.courseImgURL)"
                :icon="['fas', 'trash']"
                style="color: #9a7373"
                size="lg"
              />
        ...
        <div class="imgDisplay">
          <label for="imgList">
            <div
              class="inputLabel picContainer"
              name="imgList"
              style="cursor: pointer"
            >
              <img
                style="opacity: 0.4"
                src="/images/navbar/add-image.png"
                alt="image-gallery.png"
              />
            </div>
            <input
              ...
              @change="insertImg"
              name="imgList"
              class="form-control visually-hidden pic"
              type="file"
              id="imgList"
            />
          </label>
    ...
    <div class="courseDetailContainer">
      ...
      >
        <n-form-item path="courseName" label="課程名稱：">
          <n-input
            v-model:value="courseData.courseName"
        ...
        <n-form-item path="courseIntroduction" label="課程說明：">
          <n-input
            type="textarea"
            v-model:value="courseData.courseIntroduction"
          />
        </n-form-item>
        ...
        <n-form-item label="場地選擇：" path="hasRented">
          <n-radio-group v-model:value="hasRented" name="hasRentedOrNot">
            <n-radio-button :value="true"> 租借教室 </n-radio-button>
            <n-radio-button :value="false"> 已有場地 </n-radio-button>
          </n-radio-group>
          ...
          <n-input
            v-if="!hasRented"
            v-model:value="courseData.coursePlace"
            @keydown.enter.prevent
          />
        </n-form-item>
        ...
        <n-form-item label="課程狀態：" path="courseStatus">
          <n-select
            v-model:value="courseData.courseStatus"
            placeholder="Select"
  ...
  >
    <n-button
      ...
      type="primary"
      @click="submitChange"
      :disabled="updateDisabled ? false : true"
    >
      送出
    </n-button>
    <n-button
